...
\label{sub:training_setup}
	As stated, the fine-tuning setup follows three stages. In the first stage, the model $M$ is fine-tuned on a German-language (\textit{deu}) treebank to instill general knowledge of the task at hand. In the second stage, $M$ is fine-tuned separately on treebanks of auxiliary languages $\ell \in \{\textit{nld}, \textit{nor}, \textit{ces}, \textit{fin}\}$\footnote{Dutch, Norwegian, Czech, and Finnish.} to obtain four instances of $M^\ell$. Finally, each $M^\ell$ is fine-tuned on each target language $\lambda \in \{\textit{gsw}, \textit{fao}, \textit{hsb}, \textit{vep}\}$,\footnote{Alemannic, Faroese, Upper Sorbian, and Veps.} resulting in 16 instances of \m{\ell}{\lambda}.
